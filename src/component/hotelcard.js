...
export default function Hotelcard({Data}){
     return(
        ...
        <div className="max-w-[280px] mb-2 transform transition duration-200 hover:scale-95 ">
            ...
            <img className="w-70 h-45 object-cover rounded-xl relative" src={"https://media-assets.swiggy.com/swiggy/image/upload/" +
            (Data?.info?.cloudinaryImageId)}></img>


            
            <div className="max-w-[95%] mx-auto mt-3">
                
                <p className=" text-xl    font-bold">{Data?.info?.name}</p>
                ...
                <span className=" text-lg font-semibold  overflow-hidden">{Data?.info?.sla?.slaString}</span>
                </div>
                <div className="text-base overflow-hidden">{Data?.info?.cuisines}</div>
                <div className="text-base">{Data?.info?.areaName}</div>
            </div>
